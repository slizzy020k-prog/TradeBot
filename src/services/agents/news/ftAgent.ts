...
export class FTNewsAgent {
  private sourceName: string = 'Financial Times';
  private baseUrl: string = 'https://ft.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[FT] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[FT] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const linkPattern = /<a[^>]*href="(\/content\/[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
export const ftNewsAgent = new FTNewsAgent();