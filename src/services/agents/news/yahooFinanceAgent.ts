...
export class YahooFinanceNewsAgent {
  private sourceName: string = 'Yahoo Finance';
  private baseUrl: string = 'https://finance.yahoo.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[YahooFinance] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[YahooFinance] Failed to fetch news: ${error}`);
      return [];
    }
  }
...
export const yahooFinanceNewsAgent = new YahooFinanceNewsAgent();