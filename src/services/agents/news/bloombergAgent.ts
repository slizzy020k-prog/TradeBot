...
export class BloombergNewsAgent {
  private sourceName: string = 'Bloomberg';
  private baseUrl: string = 'https://bloomberg.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[Bloomberg] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[Bloomberg] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const titleMatch = response.title || '';
    const urlMatch = response.url || '';

    const articlePatterns = [
      /<article[^>]*>[\s\S]*?<h1[^>]*>([^<]+)<\/h1>[\s\S]*?<\/article>/gi,
      /<div[^>]*class="[^"]*article[^"]*"[^>]*>[\s\S]*?<h[1-3][^>]*>([^<]+)<\/h[1-3]>[\s\S]*?<\/div>/gi,
      /<a[^>]*href="(\/news\/[^"]+)"[^>]*>[^<]*<span[^>]*>([^<]+)<\/span>/gi,
    ];
    ...
    const linkPattern = /<a[^>]*href="(\/news\/[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
export const bloombergNewsAgent = new BloombergNewsAgent();