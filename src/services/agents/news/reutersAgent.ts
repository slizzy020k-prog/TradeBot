...
export class ReutersNewsAgent {
  private sourceName: string = 'Reuters';
  private baseUrl: string = 'https://reuters.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[Reuters] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[Reuters] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const articlePatterns = [
      ...
      /<div[^>]*class="[^"]*story[^"]*"[^>]*>[\s\S]*?<h[1-3][^>]*>\s*<a[^>]*href="([^"]+)"[^>]*>([^<]+)<\/a>[\s\S]*?<\/div>/gi,
...
export const reutersNewsAgent = new ReutersNewsAgent();