...
export class MotleyFoolNewsAgent {
  private sourceName: string = 'Motley Fool';
  private baseUrl: string = 'https://fool.com';

  async scrape(headlinesCount: number = 10): Promise<NewsAnalysis[]> {
    const url = `${this.baseUrl}/investing-news`;
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[MotleyFool] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[MotleyFool] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const articlePatterns = [
      ...
      /<div[^>]*class="[^"]*article[^"]*"[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>([^<]+)<\/a>[\s\S]*?<\/div>/gi,
      /<a[^>]*href="(\/investing[^"]+)"[^>]*>[^<]*<span[^>]*>([^<]+)<\/span>/gi,
    ];
    ...
    const linkPattern = /<a[^>]*href="(\/investing[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
export const motleyFoolNewsAgent = new MotleyFoolNewsAgent();