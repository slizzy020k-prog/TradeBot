...
export class MarketWatchNewsAgent {
  private sourceName: string = 'MarketWatch';
  private baseUrl: string = 'https://marketwatch.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[MarketWatch] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[MarketWatch] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const linkPattern = /<a[^>]*href="(\/story\/[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
export const marketWatchNewsAgent = new MarketWatchNewsAgent();