...
export class BenzingaNewsAgent {
  private sourceName: string = 'Benzinga';
  private baseUrl: string = 'https://benzinga.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[Benzinga] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[Benzinga] Failed to fetch news: ${error}`);
      return [];
    }
  }
...
export const benzingaNewsAgent = new BenzingaNewsAgent();