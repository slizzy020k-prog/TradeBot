...
export class SeekingAlphaNewsAgent {
  private sourceName: string = 'Seeking Alpha';
  private baseUrl: string = 'https://seekingalpha.com';
  ...
  private async fetchAndAnalyze(url: string, limit: number): Promise<NewsAnalysis[]> {
    try {
      ...
      logger.info(`[SeekingAlpha] Analyzed ${analyses.length} headlines`);
      return analyses;
    } catch (error) {
      logger.error(`[SeekingAlpha] Failed to fetch news: ${error}`);
      return [];
    }
  }
  ...
  private extractHeadlines(response: ScrapingResponse): Array<{title: string; url: string; summary: string}> {
    ...
    const linkPattern = /<a[^>]*href="(\/article\/[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
export const seekingAlphaNewsAgent = new SeekingAlphaNewsAgent();